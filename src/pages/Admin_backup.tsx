...
function DashboardTab() {
  ...
  useEffect(() => {
    ...
    api.fetchBookings().then(b => setRecentBookings(b.filter((x: any) => x.status === 'Pending').slice(0, 3))).catch(() => { });
  }, []);
  ...
  const statItems = [
    ...
    { label: 'Pending Bookings', value: stats?.pendingBookings || 0, icon: BookOpen, color: 'bg-yellow-50 text-yellow-600' },
  ];
...
const [dragging, setDragging] = useState(false);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    let imageUrl = form.url || '/images/conference-hall.jpg';
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="bg-gray-50 rounded-xl p-6 mb-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[#1F85A8] mb-1">Upload Image (or paste URL)</label>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">You can upload a file or paste an external image URL. Uploaded files take precedence.</p>
    ...
    <div>
      ...
      {bookings.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-2 gap-6">
            ...
            {bookings.map(b => (
              <div key={b.id} onClick={() => { setSelected(b); setShowReplyForm(false); }}
                ...
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-[#1F85A8] text-sm">{b.institutionName}</span>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColor[b.status] || 'bg-gray-100 text-gray-700'}`}>{b.status}</span>
                </div>
                <p className="text-sm text-gray-700">{b.firstName} {b.lastName}</p>
                <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                  <span>{b.eventType}</span><span>·</span>
                  <span>{b.participants} participants</span><span>·</span>
                  <span>{b.startDate}</span>
                </div>
                {b.refNumber && <p className="text-xs text-blue-500 mt-1 font-mono">{b.refNumber}</p>}
              </div>
            ))}
          </div>
          ...
          {/* Detail */}
          {selected ? (
            <div className="bg-gray-50 rounded-xl p-6 overflow-y-auto max-h-[600px]">
